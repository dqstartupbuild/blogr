...
14. Replicate image generation creates the number of images chosen in settings when `REPLICATE_API_TOKEN` exists. Multiple images are generated one after another so every requested image gets its own model run. Each image run explicitly polls until Replicate reports completion. Image-only HTTP 429 responses retry the same request up to three times, respecting `Retry-After` up to 60 seconds and otherwise waiting 10 seconds. Failed runs or completed runs without a usable image URL are logged with safe diagnostics including the model, pipeline stage, error name, HTTP status, and network code when available. Prompts, credentials, request details, and provider response bodies are excluded. Failed images are skipped while the remaining images continue.
