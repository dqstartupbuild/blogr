...
The active product's Settings tab shows site scanning, editable product details, article settings, and the publishing setup guide beside the connection fields. Product details include the product basics, features, pricing, offers, colors, and external app or extension links. Users can copy an integration prompt for the receiving app, inspect the expected webhook path and sample payload, then save the deployed webhook URL and token on that product. Product setup is not repeated on the Topics or Blogs tabs.
